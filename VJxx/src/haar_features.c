...
int vjxx_haar_x2(
	...
	unsigned int right_value = vjxx_sum_area(img, x0 + half_width + 1, y0, x0 + width, y0 + height);
...
int vjxx_haar_y2(
	...
	unsigned int bottom_value = vjxx_sum_area(img, x0, y0 + half_height + 1, x0 + width, y0 + height);
...
int vjxx_haar_x3(
  ...
  unsigned int middle_value = vjxx_sum_area(img, x0 + third_width + 1, y0, x0 + third_width * 2 + 1, y0 + height);
  unsigned int right_value = vjxx_sum_area(img, x0 + third_width * 2 + 2, y0, x0 + width, y0 + height);
...
int vjxx_haar_y3(
  ...
  unsigned int middle_value = vjxx_sum_area(img, x0, y0 + third_height + 1, x0 + width, y0 + third_height * 2 + 1);
  unsigned int bottom_value = vjxx_sum_area(img, x0, y0 + third_height * 2 + 2, x0 + width, y0 + height);
